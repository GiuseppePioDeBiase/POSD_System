...
function POSD() {
    ...
    const [selectedCWE, setSelectedCWE] = useState(null); // Stato per il CWE
    const navigate = useNavigate();
    const [menuOpen, setMenuOpen] = useState(false);
    const [strategiesOpen, setStrategiesOpen] = useState(false);
    const [isoPhaseOpen, setIsoPhaseOpen] = useState(false);
    const [articleOpen, setArticleOpen] = useState(false);
    const [cweOpen, setCweOpen] = useState(false); // Stato per aprire il menu del CWE
    ...
    const filteredPatterns = patterns.filter(pattern => {
        const matchesMVC = selectedMVC ? pattern['Collocazione MVC'] && pattern['Collocazione MVC'].includes(selectedMVC) : true;
        const matchesStrategy = selectedStrategy ? pattern['Strategies'] && pattern['Strategies'].includes(selectedStrategy) : true;
        const matchesISOPhase = selectedISOPhase ? pattern['ISO 9241-210 Phase'] && pattern['ISO 9241-210 Phase'].includes(selectedISOPhase) : true;
        const matchesArticle = selectedArticle ? pattern['Article GDPR Compliance with the Pattern'] && pattern['Article GDPR Compliance with the Pattern'].includes(selectedArticle) : true;
        const matchesCWE = selectedCWE ? pattern['CWE Top 25 Most Dangerous Software Weaknesses OWASP Categories Associated'] && pattern['CWE Top 25 Most Dangerous Software Weaknesses OWASP Categories Associated'].includes(selectedCWE) : true;
        return matchesMVC && matchesStrategy && matchesISOPhase && matchesArticle && matchesCWE;
    });
    ...
    return (
        <div className="container mx-auto mt-5 p-6 flex flex-col items-center">
            <div className="search-dropdown-container">
                ...
                <div className="sec-center">
                    <input className="dropdown" type="checkbox" id="dropdown-strategies" name="dropdown-strategies" checked={strategiesOpen}
                    ...
                    <div className={`section-dropdown ${strategiesOpen ? 'open' : ''}`}>
                        <button className={`${selectedStrategy === 'Control' ? 'selected' : ''}`} onClick={() => handlePatternClick("Control")}>Control <i className="uil uil-arrow-right"></i></button>
                        <button className={`${selectedStrategy === 'Abstract' ? 'selected' : ''}`} onClick={() => handlePatternClick("Abstract")}>Abstract <i className="uil uil-arrow-right"></i></button>
                        <button className={`${selectedStrategy === 'Separate' ? 'selected' : ''}`} onClick={() => handlePatternClick("Separate")}>Separate <i className="uil uil-arrow-right"></i></button>
                        <button className={`${selectedStrategy === 'Hide' ? 'selected' : ''}`} onClick={() => handlePatternClick("Hide")}>Hide <i className="uil uil-arrow-right"></i></button>
                        <button className={`${selectedStrategy === 'Minimize' ? 'selected' : ''}`} onClick={() => handlePatternClick("Minimize")}>Minimize <i className="uil uil-arrow-right"></i></button>
                        <button className={`${selectedStrategy === 'Inform' ? 'selected' : ''}`} onClick={() => handlePatternClick("Inform")}>Inform <i className="uil uil-arrow-right"></i></button>
                        <button className={`${selectedStrategy === 'Enforce' ? 'selected' : ''}`} onClick={() => handlePatternClick("Enforce")}>Enforce <i className="uil uil-arrow-right"></i></button>
                    </div>
                </div>

                <div className="sec-center">
                    <input className="dropdown" type="checkbox" id="dropdown-iso" name="dropdown-iso" checked={isoPhaseOpen}
                        onChange={handleIsoPhaseClick} />
                ...
                <div className="sec-center">
                    <input className="dropdown" type="checkbox" id="dropdown-article" name="dropdown-article" checked={articleOpen}
                        onChange={handleArticlePhaseClick} />
                    <label className="for-dropdown" htmlFor="dropdown-article">Article <i className="uil uil-arrow-down"></i></label>
                ...
                <div className="sec-center">
                    <input className="dropdown" type="checkbox" id="dropdown-cwe" name="dropdown-cwe" checked={cweOpen}
                        onChange={handleCweClick} />
                    ...
                    <div className={`section-dropdown ${cweOpen ? 'open' : ''}`}>
                        ...
                        <button className={`${selectedCWE === 'CWE-918' ? 'selected' : ''}`} onClick={() => handleCWEClick('CWE-918')}>CWE-918</button>
                    </div>
                </div>
